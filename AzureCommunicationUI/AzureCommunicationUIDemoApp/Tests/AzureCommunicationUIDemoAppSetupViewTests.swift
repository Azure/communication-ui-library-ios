...
class AzureCommunicationUIDemoAppSetupViewTests: XCUITestBase {
    ...
    func testCallCompositeSetupCallGroupCallSwiftUI() {
        ...
        if #unavailable(iOS 14.2) {
            // there is an AppCenter issue for devices with iOS < 14.2 when
            // useCallingSDKMock = false
            // that cause testCallCompositeSetupCallGroupCallSwiftUI to fail
            // however, the test can be run locally without any issues 
            // for all supported iOS versions (including iOS < 14.2)
            startExperience()
        } else {
        ...
        wait(for:
                app.buttons[AccessibilityIdentifier
                    .joinCallAccessibilityID.rawValue])

        let audioButtonLabel = app.buttons[AccessibilityIdentifier
            .toggleMicAccessibilityID.rawValue].label
        tapButton(accessibilityIdentifier: AccessibilityIdentifier
            .toggleMicAccessibilityID.rawValue, shouldWait: true)
        check(predicate: NSPredicate(format: "label != %@", audioButtonLabel),
              for: app.buttons[AccessibilityIdentifier
                .toggleMicAccessibilityID.rawValue])

        let videoButtonLabel = app.buttons[AccessibilityIdentifier
            .toggleVideoAccessibilityID.rawValue].label
        tapButton(accessibilityIdentifier: AccessibilityIdentifier
            .toggleVideoAccessibilityID.rawValue)

        let audioDeviceButtonValue = app.buttons[AccessibilityIdentifier
            .toggleAudioDeviceAccessibilityID.rawValue].value as? String
        tapButton(accessibilityIdentifier: AccessibilityIdentifier
            .toggleAudioDeviceAccessibilityID.rawValue,
                  shouldWait: true)
