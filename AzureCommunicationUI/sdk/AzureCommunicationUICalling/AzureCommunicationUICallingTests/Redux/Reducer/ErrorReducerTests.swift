...
class ErrorReducerTests: XCTestCase {
    ...
    func test_handleErrorReducer_reduce_when_notErrorState_then_return() {
        ...
        let action = ErrorAction.FatalErrorUpdated(error: CallErrorEvent(code: "",
                                                                     error: nil))
        let sut = getSUT()
    ...
    func test_handleErrorReducer_reduce_when_fatalErrorUpdated_then_returnErrorState_categoryFatal() {
        let state = ErrorState(error: CallErrorEvent(code: CallErrorCode.callJoin,
                                                 error: nil),
                               errorCategory: .callState)
        let errorEvent = CallErrorEvent(code: CallErrorCode.callJoin, error: nil)
    ...
    func test_handleErrorReducer_reduce_when_statusErrorCallEvictionAndCallReset_then_returnErrorState_categoryCallState() {
        let state = ErrorState(error: CallErrorEvent(code: CallErrorCode.callEvicted,
                                                 error: nil),
                               errorCategory: .callState)
        let errorEvent = CallErrorEvent(code: CallErrorCode.callEvicted, error: nil)
    ...
    func test_handleErrorReducer_reduce_when_statusErrorCallDeniedAndCallReset_then_returnErrorState_categoryCallState() {
        let state = ErrorState(error: CallErrorEvent(code: CallErrorCode.callDenied,
                                                 error: nil),
                               errorCategory: .callState)
        let errorEvent = CallErrorEvent(code: CallErrorCode.callDenied, error: nil)
    ...
    func test_handleErrorReducer_reduce_when_callStartRequested_then_cleanup() {
        let error = CallErrorEvent(code: "", error: nil)
