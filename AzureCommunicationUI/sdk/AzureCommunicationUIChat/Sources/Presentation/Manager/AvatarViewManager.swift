...
//        guard state.chatState.status == .disconnected ||
//                state.errorState.errorCategory == .chatState else {
//            return
//        }
    ...
    func set(remoteParticipantViewData: ParticipantViewData,
        ...
        let participantsList = store.state.participantsState.participantsInfoMap.values
        let idStringValue = identifier.stringValue
        guard participantsList.contains(where: { $0.id == idStringValue })
        else {
